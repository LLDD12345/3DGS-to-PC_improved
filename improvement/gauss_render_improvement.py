...
class GaussPythonRenderer():
    ...
    def __init__(self, means3D, opacity, colour, cov3d, white_bkgd=True, visible_gaussian_threshold = 0.0):
        ...
        # ======================================
        # ✅ 修复1：颜色初始化从纯黑改为NaN
        # 原代码：self.gaussian_colours = torch.zeros((means3D.shape[0], 3), device=self.device, dtype=torch.double)
        # 作用：准确区分"被渲染过"和"从未被任何相机看到"的高斯
        # ======================================
        self.gaussian_colours = torch.full((means3D.shape[0], 3), float('nan'), device=self.device, dtype=torch.double)
    ...
    def get_gaussian_colours(self):
        ...
        """
        # ======================================
        # ✅ 修复2：NaN安全处理，防止保存PLY时出错
        # 作用：将未被渲染的高斯颜色转为0，方便后续过滤
        # ======================================
        return torch.nan_to_num(self.gaussian_colours, nan=0.0) * 255
