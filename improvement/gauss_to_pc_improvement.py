...
# ==================== 新增：保留密集主体点云（内存优化版） ====================
def keep_dense_points(points, colours, normals, eps=0.1, min_points=100, voxel_size=0.05):
    """
    使用体素降采样 + DBSCAN + 最近邻映射提取最大连通区域（主体物体），避免内存爆炸。
    Args:
        points: torch.Tensor (N,3)
        colours: torch.Tensor (N,3)
        normals: torch.Tensor (N,3) or None
        eps: DBSCAN 聚类半径（米），根据场景尺度调整
        min_points: DBSCAN 形成聚类的最小点数
        voxel_size: 体素降采样大小（米），先降采样再聚类，大幅减少内存
    Returns:
        filtered_points, filtered_colours, filtered_normals
    """
    import open3d as o3d
    ...
    pcd.colors = o3d.utility.Vector3dVector(colours.cpu().numpy() / 255.0)
    
    # 体素降采样（关键：减少点数，使 DBSCAN 可行）
    down_pcd = pcd.voxel_down_sample(voxel_size)
    print(f"体素降采样：原始点数 {len(pcd.points)} -> {len(down_pcd.points)} 点")
    
    # 在降采样后的点云上执行 DBSCAN
    labels = np.array(down_pcd.cluster_dbscan(eps=eps, min_points=min_points))
    if len(labels) == 0 or np.max(labels) < 0:
        print("警告：DBSCAN 未找到有效聚类，返回原始点云")
        return points, colours, normals
    
    # 统计各聚类点数，保留最大聚类
    unique_labels, counts = np.unique(labels[labels >= 0], return_counts=True)
    if len(unique_labels) == 0:
        print("警告：没有有效聚类，返回原始点云")
        return points, colours, normals
    
    largest_cluster_label = unique_labels[np.argmax(counts)]
    mask_down = (labels == largest_cluster_label)
    
    # 提取降采样后属于最大聚类的点
    cluster_down = down_pcd.select_by_index(np.where(mask_down)[0])
    cluster_pts = np.asarray(cluster_down.points)
    if len(cluster_pts) == 0:
        print("警告：降采样后聚类为空，返回原始点云")
        return points, colours, normals
    
    # 使用最近邻搜索将原始点云中距离主体点云 eps 以内的点保留
    orig_pts = np.asarray(pcd.points)
    tree = KDTree(cluster_pts)
    distances, _ = tree.query(orig_pts, k=1)
    mask_orig = distances <= eps
    
    filtered_pcd = pcd.select_by_index(np.where(mask_orig)[0])
    
    filtered_points = torch.from_numpy(np.asarray(filtered_pcd.points)).to(points.device)
    filtered_colours = torch.from_numpy(np.asarray(filtered_pcd.colors) * 255).to(colours.device)
    if normals is not None:
        filtered_normals = normals[mask_orig]
    else:
        filtered_normals = None
    
    print(f"密集主体提取完成：原始 {len(points)} -> 保留 {len(filtered_points)} 点 ({100*len(filtered_points)/len(points):.1f}%)")
    return filtered_points, filtered_colours, filtered_normals
# ========================================================================
...
def config_parser():
    ...
    # ========== 新增：保留密集主体点云的参数 ==========
    parser.add_argument("--keep_dense_only", action="store_true", help="只保留密集的主体点云（使用DBSCAN聚类提取最大连通区域）")
    parser.add_argument("--dbscan_eps", type=float, default=0.1, help="DBSCAN聚类半径（米），根据场景调整，例如室外大场景用0.2-0.5")
    parser.add_argument("--dbscan_min_points", type=int, default=100, help="DBSCAN形成聚类的最小点数")
    parser.add_argument("--dense_voxel_size", type=float, default=0.05, help="提取主体前先体素降采样的大小（米），减小内存占用，默认0.05")
    # =================================================
...
def main():
    ...
    # ========== 新增：只保留密集主体点云 ==========
    if args.keep_dense_only:
        if not args.quiet:
            print("提取密集主体点云（移除稀疏背景区域）...")
        dense_points, dense_colours, dense_normals = keep_dense_points(
            total_point_cloud.points, total_point_cloud.colours, total_point_cloud.normals,
            eps=args.dbscan_eps, min_points=args.dbscan_min_points, voxel_size=args.dense_voxel_size
        )
        total_point_cloud = PointCloudData(points=dense_points, colours=dense_colours, normals=dense_normals)
    # =============================================
